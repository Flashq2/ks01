...
class ItemsController extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->modal_path = 'App\Models\ItemsModels';
        $this->blade_path = 'admin.menu_group.menu_group';
        $this->options = array('cluster' => 'ap1', 'encrypted' => true);
        $this->new_modal = new App\Models\ItemsModels();
        $this->blade_path = 'admin.items.items';
        $this->primary_key =  $this->new_modal->getKeyName();
        $this->page_url = url('items/items_card?type=up&code=');
        $this->blade_card = 'admin.items.items_card' ;
    ...
    public function index()
    {
        ...
        $records = App\Models\ItemsModels::paginate($this->pagination);
        $param = [
            'page' => $page,
            'tabe_id' => $tabe_id,
            'page_id' => $page_id,
    ...
    public function createData(Request $request)
    {
        try {
            ...
            if (isset($input['type']) && $input['type'] == 'edit') {
                $record = App\Models\ItemsModels::where($this->primary_key, trim(decryptHelper($input['id'])))->first();
    ...
    public function submitData(Request $request)
    {
        ...
        try {
            ...
            $primary_key  = $this->primary_key;
            $prefix = $this->prefix;
            $record = new App\Models\ItemsModels();
    ...
    public function editData(Request $request)
    {
        ...
        try {
            ...
            $record = ItemsModels::where($this->primary_key, $data['is_code'])->first();
    ...
    public function deleteData(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $record = App\Models\ItemsModels::where($this->primary_key, $data['code'])->first();
    ...
    public function ajaxPagination(Request $request)
    {
        try {
            ...
            $records = App\Models\ItemsModels::whereRaw($query)->paginate($this->pagination);
            $page = $this->page;
            $tabe_id = $this->page_id;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
    ...
    public function downLoadExcel(Request $request, $request_table)
    {
        try {
            ...
            $records = App\Models\ItemsModels::whereRaw($query)->get();
            $page = $this->page;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
            $fields = $this->system->getField($this->page_id);
    ...
    public function ImportExcel(Request $request){
        try{
            ...
            foreach($main_data as $sub_collection){
                $insert_record = new App\Models\ItemsModels() ;
                 foreach($sub_collection as $key=>$value){
                     if($this->system->hasColumn($this->prefix,$key)) {
                         $insert_record[$key] = $value;
                     }
                 }
            $record_exist = App\Models\ItemsModels::where($this->primary_key,$insert_record[$this->primary_key ?? ''])->first();
            if($record_exist){
    ...
    public function prePublishItem(Request $request){
        try{
            $data = $request->all() ;
            $item_code = $data['code'];
