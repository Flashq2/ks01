...
use App\Models\CurrencyExchangeRateModels;
...
class CurrencyExchangeRateController extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->page = "";
        $this->prefix = "currency_exchange_rate";
        $this->page_id = '10021';
        $this->page_card_id = '1002';
        $this->modal_path = 'App\Models\App\Models\CurrencyExchangeRateModels';
        $this->blade_path = 'admin.menu_group.menu_group';
        $this->options = array('cluster' => 'ap1', 'encrypted' => true);
        $this->new_modal = new App\Models\CurrencyExchangeRateModels();
        $this->blade_path = 'admin.currency_exchange_rate.currency_exchange_rate';
    ...
    public function index()
    {
        ...
        $records = App\Models\CurrencyExchangeRateModels::paginate($this->pagination);
        $param = [
            'page' => $page,
            'tabe_id' => $tabe_id,
            'page_id' => $page_id,
    ...
    public function createData(Request $request)
    {
        try {
            ...
            if (isset($input['type']) && $input['type'] == 'edit') {
                $record = App\Models\CurrencyExchangeRateModels::where($this->primary_key, trim(decryptHelper($input['id'])))->first();
    ...
    public function submitData(Request $request)
    {
        ...
        try {
            ...
            $primary_key  = $this->primary_key;
            $prefix = $this->prefix;
            $record = new App\Models\CurrencyExchangeRateModels();
    ...
    public function editData(Request $request)
    {
        ...
        try {
            ...
            $record = App\Models\CurrencyExchangeRateModels::where($this->primary_key, $data['is_code'])->first();
    ...
    public function deleteData(Request $request)
    {
        ...
        try {
            $data = $request->all();
            $record = App\Models\CurrencyExchangeRateModels::where($this->primary_key, $data['code'])->first();
            if (!$record)   return response()->json(['status' => 'warning', 'msg' => 'Record not found in databases ']);
            $record->delete();
            DB::commit();
    ...
    public function ajaxPagination(Request $request)
    {
        try {
            ...
            $records = App\Models\CurrencyExchangeRateModels::whereRaw($query)->paginate($this->pagination);
            $page = $this->page;
            $tabe_id = $this->page_id;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
    ...
    public function downLoadExcel(Request $request, $request_table)
    {
        try {
            ...
            $records = App\Models\CurrencyExchangeRateModels::whereRaw($query)->get();
            $page = $this->page;
            $page_id = $this->page_id;
            $prefix = $this->prefix;
            $fields = $this->system->getField($this->page_id);
    ...
    public function ImportExcel(Request $request){
        try{
            ...
            foreach($main_data as $sub_collection){
                $insert_record = new App\Models\CurrencyExchangeRateModels() ;
                 foreach($sub_collection as $key=>$value){
                     if($this->system->hasColumn($this->prefix,$key)) {
                         $insert_record[$key] = $value;
                     }
                 }
            $record_exist = App\Models\CurrencyExchangeRateModels::where($this->primary_key,$insert_record[$this->primary_key ?? ''])->first();
            if($record_exist){
